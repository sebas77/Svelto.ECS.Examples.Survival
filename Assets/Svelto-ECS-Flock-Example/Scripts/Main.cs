...
using Svelto.ECS.Example.Flock.Entities;
...
namespace Svelto.ECS.Example.Flock
{
    public class Main : MonoBehaviour
    {
        ...
        void InstantiateBirds(IEntityFactory factory)
        {
            ...
            cameraObj.name = "target";
            cameraControl.Target = cameraObj;
            var boidDescriptor2 = new BoidEntityDescriptor(cameraObj.GetComponents<IImplementor>());
            factory.BuildEntity(cameraObj.GetInstanceID(), boidDescriptor2);

            sts.BirdsCount = 8 * 4000;
            ...
            MathTools.FillSquareUniform(sts.BirdsCount, delegate (int x, int y)
            {
                if (x != 0 || y != 0)
                {
                    var obj = InstantiateBird(
                       cameraObj.position + new Vector3(size * x, size * y, Random.Range(-size, size)),
                       MathTools.RandomYawPitchRotation(),
                       sts);

                    var boidDescriptor = new BoidEntityDescriptor(obj.GetComponents<IImplementor>());

                    factory.BuildEntity(obj.GetInstanceID(), boidDescriptor);
                }
            });

            var descriptor = new ConfigurationEntityDescriptor(new[] { new SettingsImplementor(sts) });

            factory.BuildEntity(0, descriptor);
        }

        GameObject InstantiateBird(Vector3 position, Quaternion rotation, BoidSettingsEx boidSettings)
        {
            var obj = (GameObject)GameObject.Instantiate(birdPrefab, position, rotation);
        ...
        public void Run(IEntityFactory factory)
        { 
            InstantiateBirds(factory);
        }
        ...
        readonly XmlSerializer serializer = new XmlSerializer(typeof(Settings));
        string SettingsFileName { get { return "Settings"; } }
        string SettingsFilePath { get { return Application.dataPath + Path.DirectorySeparatorChar + "Svelto-ECS-Flock-Example\\Resources" + Path.DirectorySeparatorChar + SettingsFileName + ".xml"; } }
        ...
        void GuiBoidSettings()
        {
            ...
            var newInstancePointNum = guiTools.Switcher(settings.instancePointNum, "Instance point", new string[] { "Freedom","Box" });
