...
namespace Svelto.ECS
{
    public class EngineEntityViewDB : IEngineEntityViewDB
    {
        ...
        public T[] QueryEntityViewsAsArray<T>(out int count) where T : struct
        {
            var type = typeof(T);
            count = 0;
        ...
        public T[] QueryGroupedEntityViewsAsArray<T>(int @group, out int count) where T : struct
        {
            var type = typeof(T);
            count = 0;
        ...
        public bool TryQueryEntityView<T>(int ID, out T entityView) where T:EntityView
        {
            var type = typeof(T);
        ...
        public T QueryEntityView<T>(int ID) where T:EntityView
        {
            var type = typeof(T);
