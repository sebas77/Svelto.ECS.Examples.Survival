...
namespace Svelto.ECS
{
    public partial class EnginesRoot: IDisposable
    {
                                ...
                                object[] implementors) where T:IEntityDescriptor
        {
            ...
            var dic = EntityFactory.BuildEntity(entityID, _groupedEntityToAdd.current,
                                                descriptorEntitiesToBuild, implementors);

            for (var index = 0; index < descriptorEntitiesToBuild.Length; ++index)
            {
                _dataBinder.AddData(descriptorEntitiesToBuild[0]);
            }
